/* NOTE: this file is an almost exact copy of ProjectEntityUpdateHandler.js.
 * The only difference is that it imports
 * ProjectEntityMongoUpdateHandlerCanary.js. It's meant to be a short-lived
 * module, so that we can test the async/await code in production for some code
 * paths only.
 */
...
const ProjectEntityMongoUpdateHandler = require('./ProjectEntityMongoUpdateHandlerCanary')
  ...
  copyFileFromExistingProjectWithProject: wrapWithLock({
    beforeLock(next) {
      return function(
        ...
        folder_id = ProjectEntityMongoUpdateHandler._confirmFolder(
          project,
          folder_id
        )
