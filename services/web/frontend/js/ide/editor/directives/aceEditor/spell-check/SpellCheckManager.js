...
`,
  ...
  class SpellCheckManager {
    ...
    runSpellCheck() {
      let j
      let i, key, word
      const changedRowNums = this.getRowNumsForChangedLinesInViewport()
      const language = this.$scope.spellCheckLanguage
      let { words, positions } = this.getWords(changedRowNums)
    ...
    getWords(changedRowNums) {
      const lines = this.adapter.getLinesByRows(changedRowNums)
      const words = []
      const positions = []
    ...
    // Returns row numbers for dirty (i.e. with changes, need to be checked) visible lines.
    // If this is the first check, all lines are considered to be dirty/have changes.
    // Examples:
    // 1. Document with 10 lines; initial check
    //    Viewport show lines 3 to 7
    //    Expected result: [ 3, 4, 5, 6, 7 ] (i.e. the row numbers for the whole viewport)
    ...
    getRowNumsForChangedLinesInViewport() {
      const firstRowNum = this.adapter.getFirstVisibleRowNum()
      const lastRowNum = this.adapter.getLastVisibleRowNum()

      return this.changedLines.reduce((changedRowNums, curVal, curRow) => {
        if (curVal && curRow >= firstRowNum && curRow <= lastRowNum) {
          changedRowNums.push(curRow)
        }
        return changedRowNums
      }, [])
    }
