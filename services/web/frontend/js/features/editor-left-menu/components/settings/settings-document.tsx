...
export default function SettingsDocument() {
  ...
  const { compiler, rootDocId, setRootDocId } = useProjectSettingsContext()

  const validDocsOptions = useMemo(() => {
    const filteredDocs =
      docs?.filter(
        doc => (compiler == "typst" ? isValidTypFile(doc.doc.name) : isValidTeXFile(doc.doc.name)) || rootDocId === doc.doc.id
      ) ?? []
